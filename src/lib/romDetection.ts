...
const MISC = {
  ...
  FAST_EGG_HATCHING: 1 << 8, // FVX placeholder slot (was UPDATE_TYPE_EFFECTIVENESS in ZX); the real type-effectiveness setting moved to Settings.typeEffectivenessMod
...
function getMiscTweakMask(family: RomProfile["family"]): number {
  // Reusable TMs is meaningful in gens 1-4 (single-use TM games);
  // forgettable HMs is meaningful in gens 1-5 (games with HMs).
  const singleUseTmFamilies = new Set(["gen1", "gen2", "rs", "emerald", "frlg", "dp", "pt", "hgss"]);
  const hmFamilies = new Set([
    "gen1", "gen2", "rs", "emerald", "frlg", "dp", "pt", "hgss", "bw", "bw2",
  ]);
  const tmHmExtras = (singleUseTmFamilies.has(family) ? MISC.REUSABLE_TMS : 0)
    | (hmFamilies.has(family) ? MISC.FORGETTABLE_HMS : 0);

  switch (family) {
    case "gen1":
      return tmHmExtras | MISC.LOWER_CASE_POKEMON_NAMES | MISC.FASTEST_TEXT |
    ...
    case "rs":
      return tmHmExtras | MISC.LOWER_CASE_POKEMON_NAMES | MISC.NATIONAL_DEX_AT_START |
    ...
    case "dp":
      return tmHmExtras | MISC.LOWER_CASE_POKEMON_NAMES | MISC.RANDOMIZE_CATCHING_TUTORIAL |
    ...
    case "bw":
      return tmHmExtras | MISC.FASTEST_TEXT | MISC.BAN_LUCKY_EGG | MISC.NO_FREE_LUCKY_EGG | MISC.BAN_BIG_MANIAC_ITEMS |
        MISC.BALANCE_STATIC_LEVELS | MISC.NATIONAL_DEX_AT_START |
        MISC.RUN_WITHOUT_RUNNING_SHOES | MISC.DISABLE_LOW_HP_MUSIC;
    case "bw2":
      return tmHmExtras | MISC.FASTEST_TEXT | MISC.BAN_LUCKY_EGG | MISC.NO_FREE_LUCKY_EGG | MISC.BAN_BIG_MANIAC_ITEMS |
        MISC.NATIONAL_DEX_AT_START | MISC.RUN_WITHOUT_RUNNING_SHOES |
